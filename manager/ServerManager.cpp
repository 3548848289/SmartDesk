...
void ServerManager::commitToServer(const QModelIndex& index, QAbstractItemModel* model) {
    ...
    QString folderPath = QDir::currentPath() + "/uploads";
    QDir().mkpath(folderPath);

    QString targetPath = folderPath + '/' + QFileInfo(filePath).fileName();
    ...
    QString httpUrl = "http://192.168.5.75:5000/upload/" + QFileInfo(targetPath).fileName();
    qDebug() << httpUrl;
    ...
    file->setParent(reply); // 将 file 的生命周期交给 reply
...
void ServerManager::downloadFile(const QString& fileName) {
    ...
    QString url = QString("http://192.168.5.75:5000/download/%1/%2").arg(dir).arg(fileName);
...
void ServerManager::getCommitHistory(const QModelIndex& index, QAbstractItemModel* model) {
    // 使用 HTTP(S) 替代 FTP
    QString httpUrl = "http://192.168.5.75:5000/download/";  // 需要替换成实际的 HTTP 服务器地址
    QNetworkRequest request{QUrl(httpUrl)};
...
void ServerManager::getFilesInDirectory(const QModelIndex& index, QAbstractItemModel* model) {
    QString baseHttpUrl = "http://192.168.5.75:5000/list/";
